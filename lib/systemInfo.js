...
async function fetchHost(adapter) {
	try {
		...
		await adapter.setStateAsync("system.software.boardVersion", {
			val: host.clientVersion || "",
			ack: true,
		});
		// desktopVersion
		await adapter.setStateAsync("system.software.desktopVersion", {
			val: host.desktopVersion || "",
			ack: true,
		});
		// platform
		await adapter.setStateAsync("system.software.platform", {
			val: host.platform || "",
			ack: true,
		});
		// os (Software)
		await adapter.setStateAsync("system.software.os", {
			val: host.os || "",
			ack: true,
		});
		// kernelArch
		await adapter.setStateAsync("system.hardware.kernelArch", {
			val: host.kernelArch || "",
			ack: true,
		});
		// cpuModel (aus cpu.model)
		await adapter.setStateAsync("system.hardware.cpuModel", {
			val: host.cpu?.model || "",
			ack: true,
		});
		// hostname (Hardware)
		await adapter.setStateAsync("system.hardware.hostname", {
			val: host.hostname || "",
			ack: true,
		});
	} catch (error) {
...
async function fetchConfig(adapter) {
	try {
		...
		await adapter.setStateAsync("system.cams.cam0", { val: json, ack: true });
		await adapter.setStateAsync("system.cams.cam1", { val: json, ack: true });
		await adapter.setStateAsync("system.cams.cam2", { val: json, ack: true });
	} catch (error) {
		if (error && typeof error === "object" && typeof error.message === "string" && error.message.includes("JSON")) {
