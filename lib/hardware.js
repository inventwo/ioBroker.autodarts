...
async function init(adapter) {
	...
	await adapter.setStateAsync("online", false, true);
	await adapter.setStateAsync("status.boardStatus", { val: "offline", ack: true });
...
async function handleForeignStateChange(adapter, id, state) {
	if (adapter.config.lightTargetId && id === adapter.config.lightTargetId) {
		await adapter.setStateAsync("system.hardware.light", { val: state.val, ack: true });
		return true;
	}

	if (adapter.config.powerTargetId && id === adapter.config.powerTargetId) {
		await adapter.setStateAsync("system.hardware.power", { val: state.val, ack: true });
		return true;
	}
