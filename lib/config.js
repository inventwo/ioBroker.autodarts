...
async function initializeRuntimeValues(adapter) {
	...
	await adapter.setState("config.tripleMinScore", {
		val: adapter.tripleMinScoreRuntime,
		ack: true,
	});
	await adapter.setState("config.tripleMaxScore", {
		val: adapter.tripleMaxScoreRuntime,
		ack: true,
	});
	await adapter.setState("config.triggerResetSec", {
...
async function handleStateChange(adapter, idShort, state) {
	if (idShort === "config.tripleMinScore") {
		...
		await adapter.setState("config.tripleMinScore", { val, ack: true });
		return true;
	}
	...
	if (idShort === "config.tripleMaxScore") {
		...
		await adapter.setState("config.tripleMaxScore", { val, ack: true });
		return true;
	}
	...
	if (idShort === "config.triggerResetSec") {
		...
		await adapter.setState("config.triggerResetSec", { val, ack: true });
		return true;
	}
