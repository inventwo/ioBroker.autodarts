...
async function updateThrow(adapter, lastDart) {
	...
	if (timeoutMs > 0) {
		adapter.resetTimers["throw"] = setTimeout(async () => {
			if (isTriple) {
				await adapter.setStateAsync("trigger.isTriple", { val: false, ack: true });
			}
			if (isDouble) {
				await adapter.setStateAsync("trigger.isDouble", { val: false, ack: true });
			}
			if (isBullseye) {
				await adapter.setStateAsync("trigger.isBullseye", { val: false, ack: true });
			}
			if (isMiss) {
				await adapter.setStateAsync("trigger.isMiss", { val: false, ack: true });
			}
