...
async function init(adapter) {
	...
	await adapter.setState("tools.RAW", { val: "", ack: true });
	await adapter.setState("trigger.isBusted", { val: false, ack: true });
	await adapter.setState("trigger.isGameon", { val: false, ack: true });
	await adapter.setState("trigger.isGameshot", { val: false, ack: true });
	await adapter.setState("trigger.is180", { val: false, ack: true });
	await adapter.setState("trigger.isMatchshot", { val: false, ack: true });
	await adapter.setState("trigger.isTakeout", { val: false, ack: true });
...
async function setPulse(adapter, id, timeoutMs) {
	...
	// 1) Auf true setzen
	await adapter.setState(id, { val: true, ack: true });
...
async function updateToolsUrls(adapter) {
	...
	if (!ip) {
		await adapter.setState("tools.config.urlBusted", { val: "", ack: true });
		await adapter.setState("tools.config.urlGameon", { val: "", ack: true });
		await adapter.setState("tools.config.urlGameshot", { val: "", ack: true });
		await adapter.setState("tools.config.url180", { val: "", ack: true });
		await adapter.setState("tools.config.urlMatchshot", { val: "", ack: true });
		await adapter.setState("tools.config.urlTakeout", { val: "", ack: true });
		return;
	}
	...
	await adapter.setState("tools.config.urlBusted", { val: urlBusted, ack: true });
	await adapter.setState("tools.config.urlGameon", { val: urlGameon, ack: true });
	await adapter.setState("tools.config.urlGameshot", { val: urlGameshot, ack: true });
	await adapter.setState("tools.config.url180", { val: url180, ack: true });
	await adapter.setState("tools.config.urlMatchshot", { val: urlMatchshot, ack: true });
	await adapter.setState("tools.config.urlTakeout", { val: urlTakeout, ack: true });
